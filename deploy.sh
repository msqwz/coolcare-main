...
if [ -d "frontend" ] && [ -f "frontend/package.json" ]; then
    ...
    if ! npm install --no-audit --no-fund; then
        echo "⚠️  npm install в dispatcher завершился с ошибкой, пробую продолжить..."
    fi
    
    echo "   🏗️  Запуск vite build..."
    if ! npm run build; then
        echo "❌ Ошибка сборки Диспетчерской! (Vite не найден или ошибка в коде)"
        # Попробуем проверить наличие node_modules
        if [ ! -d "node_modules" ]; then
            echo "   ❌ Папка node_modules отсутствует. npm install не удался."
        fi
        exit 1
    fi
